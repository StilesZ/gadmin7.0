var Fkreport = {
    ...
    init: function (id) {
        return {
            id: id,//表单名称
            f_id: 'Fk' + Fkreport.dateFormat(new Date(), "yyyyMMddhhmmssS"),//表单ID
            f_list: {
                //设计数据
            },
            chart: {
    ...
    deleteElment: () => {  //通过id删除指定图层
        layui.use('layer', function () {
            ...
            layer.confirm('确认删除该图层？', {
                ...
                Fkreport.dataSave('', id, 'ChartDel');  //数据同步到本地
    ...
    set_att: function (obj, type = '') {
        ...
        layui.use(['form', 'layedit', 'laydate'], function () {
            ...
            form.on('select(d_type)', function (data) {
                let f_id = $('#f_id').val();
                Fkreport.set_class(f_id, 'd_type', '', data.value);
            });
        });
    },
